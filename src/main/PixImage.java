...
public class PixImage {
	...
	public PixImage(int width, int height) {
		...
		this.width = width;
		this.height = height;
		pixel = new image_matrix[height][width];
		for (int j = 0; j < height; ++j) {
			for (int i = 0; i < width; ++i) {
				pixel[j][i] = new image_matrix();
				pixel[j][i].red = 0;
				pixel[j][i].green = 0;
				pixel[j][i].blue = 0;
			}
	...
	public PixImage(PixImage obj) {
		...
		for (int j = 0; j < height; ++j) {
			for (int i = 0; i < width; ++i) {
				pixel[j][i] = new image_matrix();
				pixel[j][i].red = obj.pixel[j][i].red;
				pixel[j][i].green = obj.pixel[j][i].green;
				pixel[j][i].blue = obj.pixel[j][i].blue;
			}
		}
	}
	...
	public String toString() {
		...
		for (int j = 0; j < height; ++j) {
			for (int i = 0; i < width; ++i) {
				pixlimage += pixel[j][i].red;
				pixlimage += pixel[j][i].green;
				pixlimage += pixel[j][i].blue;
				pixlimage += '\t';
			}
			pixlimage += '\n';
		}
	...
	/**
	 * boxBluronce() returns a blurred version of "this" PixImage.
	 * 
	 * A pixel not on the image boundary has nine neighbors--the pixel itself
	 * and the eight pixels surrounding it. A pixel on the boundary has six
	 * neighbors if it is not a corner pixel; only four neighbors if it is a
	 ...
	 * @return a blurred version of "this" PixImage.
	 */
	public PixImage boxBluronce() {
		PixImage blur = new PixImage(this);
		short averagered, averagegreen, averageblue;
		short count;
		int i, j;
		for (i = 0; i < height; i++) {
			for (j = 0; j < width; j++) {
				averagered = pixel[i][j].red;
				averagegreen = pixel[i][j].green;
				averageblue = pixel[i][j].blue;
				count = 1;
				if (access(i - 1, j + 1)) {
					averagered += pixel[i - 1][j + 1].red;
					averagegreen += pixel[i - 1][j + 1].green;
					averageblue += pixel[i - 1][j + 1].blue;
					++count;
				}
				if (access(i, j + 1)) {
					averagered += pixel[i][j + 1].red;
					averagegreen += pixel[i][j + 1].green;
					averageblue += pixel[i][j + 1].blue;
					++count;
				}
				if (access(i + 1, j + 1)) {
					averagered += pixel[i + 1][j + 1].red;
				...
				if (access(i - 1, j)) {
					averagered += pixel[i - 1][j].red;
					averagegreen += pixel[i - 1][j].green;
					averageblue += pixel[i - 1][j].blue;
					++count;
				}
				if (access(i + 1, j)) {
					averagered += pixel[i + 1][j].red;
				...
				averagegreen /= count;
				averageblue /= count;
				blur.pixel[i][j].red = averagered;
				blur.pixel[i][j].blue = averageblue;
				blur.pixel[i][j].green = averagegreen;
			}
		}
	 ...
	 * @return boolean flag
	 */
	public boolean access(int i, int j) {
	...
	public int access_y_coordinate(int y) {
		if (y < 0) {
			return 0;
		} else if (y >= width) {
			return width - 1;
	...
	public PixImage sobelEdges() {
		// Replace the following line with your solution.
		PixImage edgedetected = new PixImage(this);
		long[][] energy = findEnergy();
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				edgedetected.pixel[i][j].red = edgedetected.pixel[i][j].green = edgedetected.pixel[i][j].blue = mag2gray(energy[i][j]);
			}
		}
		return edgedetected;
	...
	/**
	 * finds the gradient of this PixImage
	 * 
	 * @param empty
	 * @return PixImage with computed gradient
	 */
	public long[][] findEnergy() {
		long[][] gradient = new long[height][width];
		int i, j;
		int gxred, gyred, gxblue, gyblue, gxgreen, gygreen;
		for (i = 0; i < height; i++) {
			for (j = 0; j < width; j++) {
				// processing red gx
				gxred = gyred = gxblue = gyblue = gxgreen = gygreen = 0;
				gxred += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].red;
				gxred -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].red;
				gxred += 2 * pixel[access_x_coordinate(i - 1)][access_y_coordinate(j)].red;
				gxred -= 2 * pixel[access_x_coordinate(i + 1)][access_y_coordinate(j)].red;
				gxred += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].red;
				gxred -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].red;
				// processing red gy
				gyred += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].red;
				gyred += pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].red;
				gyred += 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j - 1)].red;
				gyred -= 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j + 1)].red;
				gyred -= pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].red;
				gyred -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].red;
				// processing blue gx
				gxblue = gyblue = gxblue = gyblue = gxgreen = gygreen = 0;
				gxblue += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].blue;
				gxblue -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].blue;
				gxblue += 2 * pixel[access_x_coordinate(i - 1)][access_y_coordinate(j)].blue;
				gxblue -= 2 * pixel[access_x_coordinate(i + 1)][access_y_coordinate(j)].blue;
				gxblue += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].blue;
				gxblue -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].blue;
				// processing blue gy
				gyblue += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].blue;
				gyblue += pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].blue;
				gyblue += 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j - 1)].blue;
				gyblue -= 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j + 1)].blue;
				gyblue -= pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].blue;
				gyblue -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].blue;
				// processing green gx
				gxgreen = gygreen = gxgreen = gygreen = gxgreen = gygreen = 0;
				gxgreen += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].green;
				gxgreen -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].green;
				gxgreen += 2 * pixel[access_x_coordinate(i - 1)][access_y_coordinate(j)].green;
				gxgreen -= 2 * pixel[access_x_coordinate(i + 1)][access_y_coordinate(j)].green;
				gxgreen += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].green;
				gxgreen -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].green;
				// processing green gy
				gygreen += pixel[access_x_coordinate(i - 1)][access_y_coordinate(j - 1)].green;
				gygreen += pixel[access_x_coordinate(i + 1)][access_y_coordinate(j - 1)].green;
				gygreen += 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j - 1)].green;
				gygreen -= 2 * pixel[access_x_coordinate(i)][access_y_coordinate(j + 1)].green;
				gygreen -= pixel[access_x_coordinate(i - 1)][access_y_coordinate(j + 1)].green;
				gygreen -= pixel[access_x_coordinate(i + 1)][access_y_coordinate(j + 1)].green;
				// set gradient value
				gradient[i][j] = gxred * gxred + gyred * gyred + gxgreen
						* gxgreen + gygreen * gygreen + gxblue * gxblue
						+ gyblue * gyblue;
			}
		}
		return gradient;
