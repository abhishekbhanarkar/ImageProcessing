...
public class RunLengthEncoding implements Iterable {
	...
	public RunLengthEncoding(int width, int height) {
		// Your solution here.
		image = new SList();
		image.insertFront(0, 0, 0, width * height);
		this.width = width;
		this.height = height;
	}
	...
	public RunLengthEncoding(int width, int height, int[] red, int[] green,
		...
		for (int i = 0; i < runLengths.length; i++) {
			image.insertEnd(red[i], green[i], blue[i], runLengths[i]);
		}
	}
	...
	public String toString() {
		// Replace the following line with your solution.
		String stringvalue = "{";
		SListNode current = image.getHead();
		while (current != null) {
			stringvalue += "<";
			stringvalue += current.red + " ";
			stringvalue += current.green + " ";
			stringvalue += current.blue + " ";
			stringvalue += current.runlength + " ";
			stringvalue += ">,";
	...
	public RunLengthEncoding(PixImage image) {
		...
		short red1, red2, green1, green2, blue1, blue2;
		red1 = blue1 = green1 = -1;
		SListNode current = null;
		this.image.RemoveFront();
		for (int i = 0; i < image.width; i++) {
			for (int j = 0; j < image.height; j++) {
				blue2 = image.getBlue(i, j);
				red2 = image.getRed(i, j);
				green2 = image.getGreen(i, j);
				if (compare_RGB(red1, green1, blue1, red2, green2, blue2)) {
					current.runlength = (int) current.runlength + 1;
				} else { //
					this.image.insertEnd(red2, green2, blue2, 1);
					if (this.image.size == 1) {
						current = this.image.getHead();
					} else {
						current = current.next;
					}
				}
	...
	public boolean compare_RGB(short red1, short green1, short blue1,
			short red2, short green2, short blue2) {
	...
	public void check() {
		...
		SListNode nextcurrent = current.next;
		if (nextcurrent == null) {
			if (width * height != (int) current.runlength) {
				System.out.println("Errror!!!!!!!!!!");
			}
			return;
		}
		while (current.next != null) {
			if (nextcurrent.red == current.red
					&& nextcurrent.green == current.green
					&& nextcurrent.blue == current.blue) {
				System.out.println("Errror!!!!!!!!!!");
				break;
			}
			no_of_pixels += (int) current.runlength;
			current = nextcurrent;
			nextcurrent = nextcurrent.next;
		}
		if (width * height != no_of_pixels) {
			System.out.println("Errror!!!!!!!!!!");
		}
	}
